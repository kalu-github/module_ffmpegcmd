...
#define OFFSET(x) offsetof(OptionsContext, x)
    OptionDef options[] = {
            ...
            {"t", HAS_ARG | OPT_TIME | OPT_OFFSET |
             ...
             "duration"},
            {"to", HAS_ARG | OPT_TIME | OPT_OFFSET | OPT_INPUT | OPT_OUTPUT,
    ...
    if (savedCode == 0) {
        ...
        exit_program((received_nb_signals || cancelRequested(executionId)) ? 255
                                                                           : main_ffmpeg_return_code);

    } else {
        main_ffmpeg_return_code = (received_nb_signals || cancelRequested(executionId)) ? 255
                                                                                        : longjmp_value;
    }
