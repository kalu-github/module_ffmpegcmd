...
import lib.kalu.ffmpegcmd.bean.FFmpegExecution;
...
public class FFmpeg {
    ...
    public static int execute(final List<String> arguments) {
        ...
        return FFcmd.ffmpegExecute(DEFAULT_EXECUTION_ID, list.toArray(new String[]{}));
    }


    public static int execute(final String[] arguments) {
        return FFcmd.ffmpegExecute(DEFAULT_EXECUTION_ID, arguments);
    }
...
//    }
     ...
     * @return returns a unique id that represents this execution
     */
    public static long executeAsync(final String[] arguments, final OnFFmpegChangeListener executeCallback, final Executor executor) {
        final long newExecutionId = executionIdCounter.incrementAndGet();
...
//    }
    ...
    public static long executeAsync(final String command, final OnFFmpegChangeListener executeCallback) {
        final long newExecutionId = executionIdCounter.incrementAndGet();
        String[] arguments = format(command);
        AsyncFFmpegExecuteTask asyncFFmpegExecuteTask = new AsyncFFmpegExecuteTask(newExecutionId, arguments, executeCallback);
        asyncFFmpegExecuteTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    ...
    public static long executeAsync(String[] cmds, OnFFmpegChangeListener executeCallback) {
        final long newExecutionId = executionIdCounter.incrementAndGet();
    ...
    public static long executeAsync(List<String> cmds, OnFFmpegChangeListener executeCallback) {
     ...
     * @return returns a unique id that represents this execution
     */
    public static long executeAsync(final String command, final OnFFmpegChangeListener executeCallback, final Executor executor) {
        final long newExecutionId = executionIdCounter.incrementAndGet();
        String[] arguments = format(command);
        AsyncFFmpegExecuteTask asyncFFmpegExecuteTask = new AsyncFFmpegExecuteTask(newExecutionId, arguments, executeCallback);
        asyncFFmpegExecuteTask.executeOnExecutor(executor);
    ...
    public static void cancel() {
        FFcmd.ffmpegCancel(DEFAULT_EXECUTION_ID);
    }
    ...
    public static void cancel(final long executionId) {
        FFcmd.ffmpegCancel(executionId);
    }
    ...
    public static List<FFmpegExecution> listExecutions() {
        return FFcmd.listFFmpegExecutions();
    }
    ...
    public static final String getFFmpegVersion() {
        return FFcmd.getFFmpegVersion();
    }
}
