...
static const char *const opt_name_codec_names[] = {"c", "codec", "acodec", "vcodec", "scodec",
                                                   "dcodec", NULL};
...
static const char *const opt_name_muxing_queue_data_threshold[] = {"muxing_queue_data_threshold",
                                                                   NULL};
        ...
        if (negative)
            ...
            for (i = 0; i < input_files[file_idx]->nb_streams; i++) {
                if (check_stream_specifier(input_files[file_idx]->ctx,
                                           ...
                                           *p == ':' ? p + 1 : p) <= 0)
                    continue;
                if (input_streams[input_files[file_idx]->ist_index + i]->user_set_discard ==
                    AVDISCARD_ALL) {
                    disabled = 1;
                    continue;
                }
...
static int opt_map_channel(void *optctx, const char *opt, const char *arg) {
    ...
    if (m->channel_idx < 0 || m->channel_idx >= st->codecpar->channels ||
        input_streams[input_files[m->file_idx]->ist_index + m->stream_idx]->user_set_discard ==
        AVDISCARD_ALL) {
        if (allow_unused) {
...
static int add_input_streams(OptionsContext *o, AVFormatContext *ic) {
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        ist->decoder_opts = filter_codec_opts(o->g->codec_opts, ist->st->codecpar->codec_id, ic, st,
                                              ist->dec);
...
static OutputStream *
new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index) {
    OutputStream *ost;
...
static OutputStream *
new_attachment_stream(OptionsContext *o, AVFormatContext *oc, int source_index) {
...
static int open_output_file(OptionsContext *o, const char *filename) {
    ...
    if (!o->nb_stream_maps) {
        ...
        if (!o->data_disable) {
            enum AVCodecID codec_id = av_guess_codec(oc->oformat, NULL, filename, NULL,
                                                     AVMEDIA_TYPE_DATA);
            ...
            switch (ost->enc_ctx->codec_type) {
                ...
                case AVMEDIA_TYPE_AUDIO:
                    if (ost->enc_ctx->sample_fmt != AV_SAMPLE_FMT_NONE) {
                        ...
                        if (!f->formats)
                            return exit_program(1);
                        memcpy(f->formats, ost->enc->sample_fmts,
...
void show_help_default(const char *opt, const char *arg) {
    ...
    if (show_avoptions) {
        ...
        show_help_children(av_bsf_get_class(), AV_OPT_FLAG_VIDEO_PARAM | AV_OPT_FLAG_AUDIO_PARAM |
...
const OptionDef options[] = {
        ...
        {"time_base", HAS_ARG | OPT_STRING | OPT_EXPERT | OPT_SPEC | OPT_OUTPUT,
         ...
         "ratio"},
        {"enc_time_base", HAS_ARG | OPT_STRING | OPT_EXPERT | OPT_SPEC | OPT_OUTPUT,
        ...
        {"max_muxing_queue_size", HAS_ARG | OPT_INT | OPT_SPEC | OPT_EXPERT | OPT_OUTPUT,
         ...
         "packets"},
        {"muxing_queue_data_threshold", HAS_ARG | OPT_INT | OPT_SPEC | OPT_EXPERT | OPT_OUTPUT,
