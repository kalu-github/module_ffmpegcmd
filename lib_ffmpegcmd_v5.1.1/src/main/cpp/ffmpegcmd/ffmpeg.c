...
void term_exit(void)
{
    av_log(NULL, AV_LOG_DEBUG, "%s", "term_exit");
    term_exit_sigsafe();
}
 ...
 */
static int transcode(int64_t callBackHandle, void (*callbackJNI)(int64_t, int, float),
                     int64_t totalTime) {
    int ret, i, w;
    AVFormatContext *os;
    OutputStream *ost;
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        outputStream->progressCallBack = callbackJNI;
...
int exe_ffmpeg_cmd(int argc, char **argv,
                   int64_t handle, void (*callbackJNI)(int64_t, int, float),
                   int64_t totalTime) {
    int i, ret;
...
//    avformat_network_init();
    ...
    if (transcode(handle, callbackJNI, totalTime) < 0)
        return exit_program(1);
    if (do_benchmark) {
